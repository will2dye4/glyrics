...
GLYRICS_GIT_HOME = os.getenv('GLYRICS_GIT_HOME', '/Users/jeopardye/dev/git/glyrics')
GLYRICS_LOG_FILE = os.getenv('GLYRICS_LOG_FILE', '/var/log/glyrics/server.log')
GLYRICS_PID_FILE = os.getenv('GLYRICS_PID_FILE', '/opt/homebrew/var/run/glyrics.pid')

GLYRICS_VIRTUALENV_HOME = os.getenv('GLYRICS_VIRTUALENV_HOME', '/Users/jeopardye/.virtualenvs/glyrics')
GLYRICS_VIRTUALENV_ACTIVATE = os.path.join(GLYRICS_VIRTUALENV_HOME, 'bin', 'activate')
...
class GLyricsMain:
    ...
    def parse_args(args: list[str]) -> argparse.Namespace:
        parser = argparse.ArgumentParser(description='CLI for administering the GLyrics API server.')
    ...
    def get_version_number() -> str:
        with open(os.path.join(GLYRICS_GIT_HOME, 'pyproject.toml')) as f:
            lines = f.readlines()
        for line in lines:
    ...
    def fetch_changes(cls) -> tuple[str, str]:
        ...
        subprocess.run(['git', 'pull', 'origin', 'master'], check=True, cwd=GLYRICS_GIT_HOME)
        new_version = cls.get_version_number()
        return starting_version, new_version

    @classmethod
    def fetch_and_print(cls) -> None:
        print(cyan('Fetching the latest version of glyrics from GitHub.'))
    ...
    def deploy_server(self) -> None:
        print(cyan('Preparing to fetch and deploy the latest version of the glyrics server.'))
        ...
        process = subprocess.Popen(f'. {GLYRICS_VIRTUALENV_ACTIVATE} && pip install .',
                                   cwd=GLYRICS_GIT_HOME, shell=True)
        process.wait()
        ...
        self.restart_server()
        print(green(f'\nSuccessfully deployed version {new_version} of the glyrics server.'))

    @staticmethod
    def print_logs() -> None:
        print(bold(f'Server logs (from {GLYRICS_LOG_FILE}):\n'))
        try:
            subprocess.run(['tail', '-f', GLYRICS_LOG_FILE])
        except KeyboardInterrupt:
            pass

    @classmethod
    def print_version(cls) -> None:
        print(bold(f'Current version of glyrics: {cls.get_version_number()}'))

    @staticmethod
    def get_server_pid() -> str:
        if not os.path.exists(GLYRICS_PID_FILE):
            return ''
        with open(GLYRICS_PID_FILE) as f:
            return f.read().strip()
    ...
    def print_server_pid(cls) -> None:
        ...
        if pid:
            print(bold(f'glyrics server PID: {pid}'))
        else:
            print(red('The glyrics server is not currently running.'))

    @classmethod
    def start_server(cls) -> None:
        print(cyan('Starting the glyrics server.'))
        pid = cls.get_server_pid()
        if pid:
            print(cyan(f'The glyrics server is already running (PID {pid}).'))
        else:
            subprocess.Popen(
                f'(. {GLYRICS_VIRTUALENV_ACTIVATE} &&'
                f' nohup gunicorn -c glyrics/config/gunicorn.conf.py "glyrics:app") >> {GLYRICS_LOG_FILE} 2>&1',
                cwd=GLYRICS_GIT_HOME, shell=True
            )
            time.sleep(1)
            pid = cls.get_server_pid()
            if pid:
                print(green(f'Started the glyrics server (PID {pid}).'))
            else:
                print(red('Failed to start the glyrics server!'))
                raise RuntimeError()

    @classmethod
    def stop_server(cls) -> None:
        print(cyan('Stopping the glyrics server.'))
        pid = cls.get_server_pid()
        if pid:
            print(bold(f'Stopping the server (PID {pid})...'))
            subprocess.run(['kill', '-9', pid], check=True)
            print(green(f'Successfully stopped the glyrics server.'))
            if os.path.exists(GLYRICS_PID_FILE):
                os.remove(GLYRICS_PID_FILE)
        else:
            print(cyan('The glyrics server is not currently running.'))
